...
- **Behaviour:** Tech-detail settings (similarity threshold, mean-color threshold, auto-select toggles, hash-pool re-calibration) live under a single collapsible panel rather than cluttering the main scan UI. New users see only the source list and the **Start Scan** button by default; power users expand to tune. Each setting shows a **one-line concise description**; the full technical detail is in a **hover tooltip** on the label/control (set via `setToolTip`, `scan_dialog.*_tooltip` keys) so the panel stays compact and the long text no longer truncates on narrow widths.
