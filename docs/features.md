...
- **Entry point:** ``scan.hash_pool`` key in ``settings.json`` (power-user escape hatch) **plus** the "Re-calibrate hash pool on next scan" checkbox in the Advanced-Settings panel — both read in [app/views/dialogs/scan_dialog.py](../app/views/dialogs/scan_dialog.py) at scan start (``_resolve_hash_pool``) and passed to ``ScanWorker``.
- **Trigger:** Always — value is consumed on every Start Scan.
...
- **Re-calibrate checkbox + modal (PR3c):** the Advanced-Settings "Re-calibrate hash pool on next scan" checkbox (default off) is the GUI front end for auto. **Checked** → the next scan forces a fresh measurement (ignores the cache), switches the persisted mode to ``"auto"``, and the box **auto-unchecks** — so later scans of the same library silently reuse the cached result. **Unchecked + auto + cache hit** → reuse silently. **Unchecked + auto + cache miss** (no calibration for this machine+folders yet) → a modal asks *Calibrate now* vs *Use thread (safe)*; "Use thread" runs this scan on thread without persisting, so it asks again next time. **Unchecked + thread/process** → the explicit ``settings.json`` value is used as-is. Fresh users stay on ``"thread"`` (default) and never see the modal until they opt into auto via the checkbox.
