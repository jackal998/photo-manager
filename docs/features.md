...
- **Conditions / variants:** Right-click parity — the Execute Action dialog's tree context menu and the main window's multi-selection right-click both offer **Set Action by Field/Regex…**, opening the same dialog with the same live preview. Both routes also emit the same `Decision set to '<decision>'` status-bar confirmation on Apply ([#316](https://github.com/jackal998/photo-manager/issues/316)); previously the Execute Action route left the status bar at its `Loaded manifest` baseline.
- **Related:** [PR #162](https://github.com/jackal998/photo-manager/pull/162) (Phase A — live preview, validation, right-click parity); [#316](https://github.com/jackal998/photo-manager/issues/316) (status-bar parity for the Execute Action route); QA scenarios [`qa/scenarios/s14_action_by_regex.py`](../qa/scenarios/s14_action_by_regex.py), [`qa/scenarios/s30_execute_dialog_regex_right_click.py`](../qa/scenarios/s30_execute_dialog_regex_right_click.py).
- **Last verified:** 2026-05-19 (PR for [#316](https://github.com/jackal998/photo-manager/issues/316))
