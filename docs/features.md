...
- **Last verified:** 2026-05-27 (Select-by scope + main-tree sync — [#443](https://github.com/jackal998/photo-manager/issues/443), [#444](https://github.com/jackal998/photo-manager/issues/444))
...
- **Related:** issue [#468](https://github.com/jackal998/photo-manager/issues/468); paired with graceful shutdown hook [#473](https://github.com/jackal998/photo-manager/issues/473) and the Windows Job Object work in [#460](https://github.com/jackal998/photo-manager/issues/460). The kill-on-close reaping was **silently inert from #460 through #553** — [#555](https://github.com/jackal998/photo-manager/issues/555) fixed a handle-lifetime bug (`int(PyHANDLE)` dropped the OS handle before `AssignProcessToJobObject` ran → ERROR_INVALID_HANDLE) that meant nothing was ever actually placed in the job. With that fixed, the **process-mode hash workers** (the `ProcessPoolExecutor` children — the real disk-readers behind the #549 orphan pain) now genuinely terminate on a hard parent-kill via the shared `assign_pid_to_kill_job` helper. **exiftool is intentionally NOT jailed** ([#556](https://github.com/jackal998/photo-manager/issues/556)): jailing a live `-stay_open` exiftool while the parent is itself inside a job (CI runner / console host) corrupts its extended EXIF pass, and since the #555 bug meant exiftool was never actually jailed before, leaving it un-jailed is the same behaviour every prior release shipped (it gets a graceful sentinel shutdown on normal exit anyway). Reaping exiftool on hard-exit is deferred to a follow-up that resolves the job-nesting interaction. **Graceful close during HASH** ([#561](https://github.com/jackal998/photo-manager/issues/561)): the cancel path now **hard-kills the live exiftool consumer processes** (`ExiftoolProcess.kill`) before joining them — a consumer wedged inside a 500-file `batch_read_extracts` only checks the cancel flag between queue gets, so without the kill the `join(timeout=5)` abandoned it, the `wait(3000)` overran (the "can't close" symptom), and the un-jailed exiftool orphaned. Killing the subprocess drops EOF into the consumer's reader so it unblocks immediately, the join completes fast, and nothing is left running on a dialog-close mid-scan.
- **Last verified:** 2026-06-04 (#561 — cancel during HASH hard-kills exiftool consumers; no orphan on dialog-close mid-scan)
...
- **Entry point:** ``scan.hash_pool`` key in ``settings.json`` (power-user escape hatch) **plus** the "Re-calibrate hash pool on scan" checkbox in the Advanced-Settings panel — both read in [app/views/dialogs/scan_dialog.py](../app/views/dialogs/scan_dialog.py) at scan start (``_resolve_hash_pool``) and passed to ``ScanWorker``.
- **Trigger:** Always — value is consumed on every Start Scan.
...
- **Re-calibrate checkbox + modal (PR3c):** the Advanced-Settings "Re-calibrate hash pool on scan" checkbox (default off) is the GUI front end for auto. **Checked** → the next scan forces a fresh measurement (ignores the cache), switches the persisted mode to ``"auto"``, and the box **auto-unchecks** — so later scans of the same library silently reuse the cached result. **Unchecked + auto + cache hit** → reuse silently. **Unchecked + auto + cache miss** (no calibration for this machine+folders yet) → a modal asks *Calibrate now* vs *Use thread (safe)*; "Use thread" runs this scan on thread without persisting, so it asks again next time. **Unchecked + thread/process** → the explicit ``settings.json`` value is used as-is. Fresh users stay on ``"thread"`` (default) and never see the modal until they opt into auto via the checkbox.
- **Conditions / variants:** Default stays ``"thread"`` because the Windows ``spawn`` start method re-imports PIL/rawpy per worker (~150–300ms each), a cost only worth paying on large scans. Unknown ``scan.hash_pool`` values fall back to ``"thread"`` at ``ScanWorker`` construction. ``"auto"`` calibration is skipped (→ ``thread``) when fewer than 24 files are queued (and there's no cached entry), since the spawn/pickle overhead swamps the signal on tiny scans. Cancellation in process mode stops submitting new work and cancels queued files (``cancel_futures=True``). #549(b): the process branch uses an explicit ``try/finally`` with ``shutdown(wait=False)`` rather than ``with ProcessPoolExecutor()`` — the latter's ``__exit__`` runs ``shutdown(wait=True)``, which on cancel would block the dialog's 3s teardown budget until every in-flight ``read_bytes()`` finished (a slow-HDD/large-file stall after the user confirmed exit). With ``wait=False`` the worker returns promptly and the still-running children are reaped by the #549(a) job assignment on parent exit — same ~1-file user-visible latency as thread mode.
- **Multi-device + NAS shortcut (#554):** when ``"auto"`` is active and the scan spans **two or more physical devices with at least one remote (NAS)**, the flat thread-vs-process calibration is **skipped** and ``"thread"`` (the per-device path) is chosen unconditionally. The flat calibration built a flat ``ThreadPoolExecutor`` over the sample — it never learned that #550 made thread mode per-device. On a mixed HDD+NAS scan it measured "8 threads thrashing the HDD" (slow) and under-credited thread, picking process. But process runs the old flat single pool, bypassing the per-device I/O-overlap win. That overlap (NAS-latency-bound reads overlapping HDD-seek-bound reads in separate pools) is exactly what the flat calibration cannot see and dominates on an I/O-bound mixed scan. Single-device / all-local scans are unaffected — calibration still runs and process can still be the winner.
- **Related:** [#486](https://github.com/jackal998/photo-manager/issues/486) (PR1 extracted the picklable compute path); worker contract pinned by ``tests/test_scan_worker.py::TestHashPoolSetting`` + ``TestHashPoolCalibration``; cache persistence by ``test_store_hash_pool_rates_round_trips_through_settings``; the checkbox/modal state machine by ``tests/test_scan_dialog.py::TestResolveHashPool`` + qa scenario [`qa/scenarios/s62_scan_hash_pool_recalibrate.py`](../qa/scenarios/s62_scan_hash_pool_recalibrate.py). Real cross-process spawn is validated by real-world runs, not CI.
- **Last verified:** 2026-06-04 (#554 multi-device+NAS auto guard)
