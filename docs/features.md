...
- **Conditions / variants:** Mode persists in `settings.json` under `ui.action_dialog.mode` so power users who flip to Regex once stay there. The match counter sits in a dedicated row visible in both modes — toggling mode never hides the live count, which is the primary feedback for both inputs. Match-span highlighting in the preview emboldens the matched substring in each row regardless of mode. Dialog title is "Set Action by Field" (not "Set Action by Field/Regex") — the panel inside (Simple / Regex / Numeric) acts as the implicit qualifier. When a row was highlighted at dialog open, the pre-fill seeds the Simple panel as `("contains", value)` — matches the documented default Simple op. A custom regex typed by the user is preserved across field-combo changes; only the auto-default refreshes.
