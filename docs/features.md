...
- **Trigger:** Always on.
- **Behaviour:** A Daylight·light visual theme (warm paper canvas, hairline borders, warm-accent selection, no row-hover) replaces the flat default Qt palette across the window and its dialogs. The results tree gains: colour + shape **similarity badges** for the five states (gold ★ Ref / purple 100% / blue N% / gray-dashed N*% / muted —); a **framed group** — warm band, top border, and a warm-accent left strip on each group header, closed by a bottom border under the last child; a soft red **delete-row tint** with the file name **struck through**; a **keep-worthiness mini bar** + number in the Score column; and a **clickable padlock** in the Lock column (faint outline when unlocked, solid warm-accent when locked — clicking toggles the lock; the column header shows the padlock glyph instead of the word). The preview pane footer shows an always-visible **similarity legend** decoding the five badge states.
- **Conditions / variants:** Presentation only — no change to what the app computes or to sort / column behaviour. The clickable padlock is another entry point to the same lock flow as [Context menu — Lock / Unlock](#context-menu--lock--unlock). Grayscale / colour-blind safe: badge text stays distinct without hue, the passenger state adds a dashed border, and delete is signalled by the strikethrough + tint together (not colour alone).
