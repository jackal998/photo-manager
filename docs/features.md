...
### Execute Action — scope to selected rows (only-selected entry, #410)

- **Entry point:** Action menu → **Execute Action (only selected)…** — second sibling of the plain "Execute Action…" entry in [app/views/components/menu_controller.py](../app/views/components/menu_controller.py) (`execute_action_selected_only`). Wired to `MainWindow.on_execute_action_selected_only` which calls `FileOperationsHandler.execute_action(selected_only=True)`.
- **Trigger:** User highlights one or more file rows in the **main window tree** (multi-row via `ExtendedSelection`), then clicks the (only selected) menu entry. The entry is gated on (manifest_loaded AND ≥1 file row selected) — empty selection or no manifest greys it out (handled by `MainWindow._refresh_execute_selected_only_enabled`, called from `on_tree_selection_changed` and after every `set_manifest_actions` flip).
- **Behaviour:** Scope is a kwarg through the handler call chain — NOT in-dialog state. `execute_action(selected_only=True)` builds synthetic `PhotoGroup`s containing only the selected file rows (row-level filter, not group-level) and passes them to `ExecuteActionDialog`. The dialog itself is unaware of the filter — it acts on every decided row in the groups it was given. The plain "Execute Action…" entry remains unchanged and passes `vm.groups` whole. Execute button label is **static** ("Execute") — the older `execute_button_highlighted` swap and in-dialog `_selected_file_paths` scope branch were removed in #410, because conflating scope and intent at the same affordance hid the "only selected" capability and loaded the dialog with rows the user had no intent to act on.
- **Conditions / variants:** Synthetic groups built for the (only selected) entry preserve `group_number` and `is_expanded` so the dialog tree's grouping/expansion still reads naturally. Groups with no selected rows are dropped entirely. The "ALL files will be deleted" complete-group confirm fires per the dialog's normal logic over the groups it was given — for a synthetic group with N items all decided=delete, the confirm shows ("EVERY file deleted" is true of the synthetic group, even if the original group on disk has more rows). Lock guard scans every locked delete row in the passed groups (no in-dialog scope narrowing); upstream pre-filter already excluded out-of-scope rows.
- **Related:** [#410](https://github.com/jackal998/photo-manager/issues/410) (supersedes the in-dialog scope-narrowing introduced in [PR #219](https://github.com/jackal998/photo-manager/pull/219) for [#211](https://github.com/jackal998/photo-manager/issues/211)); QA scenario [`qa/scenarios/s44_execute_highlighted_rows.py`](../qa/scenarios/s44_execute_highlighted_rows.py).
- **Last verified:** 2026-05-25 (#410)
...
- **Last verified:** 2026-05-26 (#415 reworded DeleteRegexConfirmDialog to decision-explicit copy)
