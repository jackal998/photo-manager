...
| `frontend/src/lib/rowMetrics.ts` | `frontend/src/lib/rowMetrics.test.ts` — the two density value sets and `estimateRowSize`'s four answers (72 / 78 / 52 / 56, plus the group header's own height at either density). This is the one number in layout slice R whose being wrong turns nothing red: the rendered DOM is measured afterwards so the list self-corrects visually, and what stays broken is arrow-key scroll-into-view landing a row under the sticky header — which `overscan: 10` hides until the list is long. A second test asserts each density's CLASS strings carry its own numbers (`h-[72px]`, the header's spacers == the row's thumbnail / padlock widths), because a class table and a number table that disagree are invisible to every other test. Layer 3: `qa/web/scenarios/s74_daylight_theme.py` reads the computed boxes at both densities. |
| `frontend/src/lib/density.ts` | `frontend/src/lib/density.test.ts` — default, localStorage round-trip, and the fallback for a value that is not a density (which would otherwise index the metrics table with `undefined` and render every row with no height). The store-level round-trip (`setDensity` → localStorage → re-hydrate) is in `frontend/src/store/useAppStore.test.ts`. |
| `frontend/src/lib/groupSummary.ts` | `frontend/src/lib/groupSummary.test.ts` — the two derivations open questions Q4 lets a group header make about itself: the byte total (a plain sum) and the folder suffix (the shared folder's leaf, or a count of folders when members span more than one). Table-tested including the Windows path separator and a trailing separator, both of which would otherwise yield an empty leaf that renders as a bare `·`. Layer 3: `qa/web/scenarios/s74_daylight_theme.py` asserts a total is actually rendered — the manifest never sends one, so a broken sum renders a plausible WRONG figure rather than nothing. |
| `frontend/src/i18n/useDateLocale.ts` | `frontend/src/lib/format.test.ts` — `dateLocaleFor` maps each UI locale to its BCP-47 tag (`en → en-GB` for the REPLY's `14 Mar 2021` order, `zh_TW → zh-TW`) and an unknown locale falls back to the UI default rather than to the browser. `formatDate`'s `locale` argument is OPTIONAL, which is how all three call sites came to pass `undefined` and follow the BROWSER's locale — an English UI printing 「2024年2月1日」, with nothing thrown, nothing red, and nothing visible on an en-* dev machine. Layer 3 is what actually catches that class: `qa/web/scenarios/s75_keep_best_button.py` reads the same Shot Date cell under BOTH UI locales in one run. |
| `frontend/src/components/Toast.tsx` | `frontend/src/components/Toast.test.tsx` — the single-slot undo toast Q5 put in place of a confirm dialog on "Keep best · delete rest", which makes it the whole safety net for a bulk write. Covers: both counts and their singular/plural forms, the zh_TW copy (a toast that falls back to English is the R8 defect), Undo dispatching and being disabled while one is in flight, the ~6s dismissal, the timer **pausing on hover and resuming with the REMAINDER** (not a fresh six), and a second keep-best REPLACING the toast and restarting the clock. The store half — the snapshot, the derived counts and the force-locked restore — is in `frontend/src/store/useAppStore.test.ts`; the live behaviour is `qa/web/scenarios/s75_keep_best_button.py`. |
| `frontend/src/components/Toolbar.tsx` | `frontend/src/components/Toolbar.test.tsx` — the two NEW behaviours layout slice TB adds, not the restyling (what the strip inherited from `App.tsx` is still covered by `App.test.tsx`, which renders the real `App`). The counted bulk verbs: the label carries the live selection count at 0 / 1 / 3 and in zh_TW, the verbs are disabled at zero, and each dispatches its own decision value. The danger CTA: its label carries the delete-marked count with the right singular/plural, it opens the execute flow, and at zero it is **present but disabled** still reading "Delete 0 files…" — a destructive control that disappears is one that reappears under the cursor. Plus the filter writing to the view-only store field without touching the manifest, and a class-level count of warm-filled buttons (the computed-colour version of that assertion is `s76`). |
| `frontend/src/components/StatusBar.tsx` | `frontend/src/components/StatusBar.test.tsx` — #906's two figures (count + `formatBytes` byte sum, hidden before a manifest loads and shown AT ZERO once one has), the summary `<p>` keeping its own element identity inside the 30px strip (s74 and others read the footer surface through `[data-testid=main-status-bar].closest('footer')`, and the strip must not absorb the new figures), and the density toggle writing BOTH the store and localStorage — persistence is the point of the preference, so a toggle that only changed memory would reset on reload. |
| `frontend/src/components/SimilarityLegend.tsx` | `frontend/src/components/SimilarityLegend.test.tsx` — the five-state key layout slice E pins under the preview pane. What layer 1 can decide is COMPOSITION: all five states present and in the prototype's order (a legend missing one state is worse than none — the reader concludes the missing pill means something else), each badge carrying the text `similarityLabel()` would print for that state (the rows are derived from the same two functions the table calls, so a legend cannot keep explaining a vocabulary the table stopped using), a meaning phrase per state in both locales, the 74px min-width that belongs to the legend and must not migrate into the shared badge spec, and the (border-style, weight) pairs staying distinct across all five — the grayscale invariant `similarityBadge.test.ts` pins on the spec table, re-asserted on what is actually mounted. Its computed colours and, above all, the fact that it is PINNED (the pane scrolled to its end, the legend's bottom edge still level with the pane's) are layer 3 (`qa/web/scenarios/s74_daylight_theme.py`). |
| `frontend/src/components/execute/AllDeleteBanner.tsx`, `HiddenDestructiveBanner.tsx`, `ExecuteResultSummary.tsx` | `frontend/src/components/execute/banners.test.tsx` — the copy-audit E2/E3 translation contracts (one reorderable template per banner, singular/plural count nouns, the zh_TW word order around the injected jump anchors) plus what layout slice E changed: both banners carry the Q9 caution classes and the ▲ glyph, neither carries an `amber-*`/`yellow-*` utility or a second ⚠ in its copy, the glyph is `aria-hidden` under a `role="alert"` strip, and the post-execute split survives the retone — "Files Not Found" is caution, "Files Failed to Delete" stays danger. Class-level on purpose: the computed rgb values are layer 3 (`s74`), because jsdom cannot tell a live token from a deleted one; what it CAN catch is the regression that actually happened, a banner drifting back onto a utility scale nobody owns. |
| `frontend/src/components/PreviewPane.tsx` | `frontend/src/components/PreviewPane.test.tsx` — what layout slice PV added on top of the #787 two-attempt video contract already pinned there: the 42px title, the basename as a heading (truncated, full name in `title`), the metadata table's exact five LABELS in order, the Shot date formatted in the APP's locale (the optional `locale` argument is how three call sites came to follow the BROWSER — see `i18n/useDateLocale.ts`), the decision block dispatching the same `setDecision` the row control does for both non-default values, its three stacked buttons carrying the row's words with `''` pressed as Keep (the #584 model), a locked row disabling all three with a reason in `title`, the keep-worthiness fill's width, and the score having MOVED out of the metadata table rather than been lost. The #535 invariant test moved with the scroller (now found by `[data-preview-scroll]`, since `img.parentElement` is the 4:3 frame) and gained the frame's aspect class — the second half of that fix. The computed colours, the real aspect ratio and the pane's live 320px width are layer 3 (`qa/web/scenarios/s77_preview_pane.py`); the 288 → 320 default migration is `frontend/src/lib/panelWidths.test.ts`. |
| `frontend/src/lib/toolbarShed.ts` | `frontend/src/lib/toolbarShed.test.ts` — the 1100px manifest-open shed, table-tested AT the boundary (1100 keeps, 1099 sheds), because an off-by-one means either a control vanishing at a width where the strip still fits, or the Delete CTA scrolling off the end at one where it does not. Plus the unmeasured case (`null` / `0` / `NaN` → render everything): jsdom returns zero from every layout API, and a missing measurement must never masquerade as a narrow toolbar and silently remove a control. |
| `frontend/src/lib/rowFilter.ts` | `frontend/src/lib/rowFilter.test.ts` — the toolbar filter's predicate, plus `visibleSelectedPaths`, the selector the bulk-verb LABEL and `applyBulkDecision` both read. The selection survives the filter by design, so it can name rows nobody can see; the tests pin the intersection, that the SELECTION's order is kept (not the tree's), the empty case, and that clearing the filter gives the hidden picks back. One selector for both callers is what keeps the count in the label and the count the write touches from drifting apart. Case-insensitive substring over basename AND folder (a review is driven by where a file came from as often as by what it is called), groups with no match dropped entirely, `member_count` recomputed from the survivors (the group header renders it and derives its size total from the same items), input not mutated, and — the one that is not about matching — the empty filter returning the store's array **by identity**, because every memo in `ResultTree` keys off it and a fresh copy per render would re-sort and re-flatten the tree on each unrelated keystroke. |
| `frontend/src/lib/deleteTotals.ts` | `frontend/src/lib/deleteTotals.test.ts` — #906's count and byte sum across every group. Includes a LOCKED delete row (the lock blocks writes, not the total: Execute will still delete it, so omitting it understates the danger CTA) and a row with a missing size (contributes 0 rather than turning the whole reclaim figure into "NaN"). |
| `frontend/src/lib/deleteConfirmRows.ts` | `frontend/src/lib/deleteConfirmRows.test.ts` — #917's two derivations for the Execute confirm body: the folder buckets (deterministic path order, delete rows only, a deleted Ref row carrying no `refBasename` because it is not a duplicate of itself, and a missing size contributing 0 so the subtotals still add up to the pinned total) and the reason phrase per `SimilarityKind`. The reason table is where the value domain bites: `percent` arrives nullable on BOTH the `percent` and `passenger` kinds, and the naive template renders "null% match" / "Exact duplicate of null" — visible only to whoever opens the modal on a manifest with a Live Photo MOV or a Ref-less group, i.e. never on the fixture. Both halves are tested. The **scope** argument is the third axis, and the one a reviewer caught rather than a test: the buckets must be narrowed by the same `scope_paths` the execute call sends, or "Execute (only selected)" shows a list and a total describing files it will not touch. Tested at all three scopes (a selection of 1 of 3, a filtered subset, `null`), plus the case that makes the narrowing subtle — the Ref is still named in a reason line when the Ref itself is out of scope. Rendering, pinning and the zh_TW catalog are `DeleteConfirmDialog.test.tsx`; layer 3 is `qa/web/scenarios/s13_execute_action.py` (mixed-manifest phase, both locales + the Execute-selected scope). |
| `frontend/src/lib/scanSummary.ts` | `frontend/src/lib/scanSummary.test.ts` — the menu bar's source name, derived rather than stored because E7 (recent sources) was dropped by design including its scan-history store. Table-tested: the deepest shared folder's leaf, a single shared folder, Windows separators, and the two fallbacks — rows spanning unrelated roots, and no row carrying a folder at all — both landing on the manifest file's own name, then `null` before a manifest is loaded. |
| `frontend/src/components/ResultTree.tsx` (slice TB couplings) | `frontend/src/components/ResultTree.filterDensity.test.tsx` — the filter narrowing the rendered tree (including the group header disappearing with its last row) and the density change forcing `virtualizer.measure()`. The second one is why the file exists: the call is **gated on an actual change**, because `measure()` DISCARDS the size cache, so calling it unconditionally throws away the sizes the first layout pass just measured — which turned `ResultTree.scrollMargin.test.tsx` and `ResultTree.keyboard.test.tsx` red by 16px while this was written. The mock wraps the REAL hook and patches `measure` **once per instance** (it returns the same object every render, so re-wrapping nests the counters and turns one call into N). |
...
| `qa/web/scenarios/s74_daylight_theme.py` | `web-scenario-batch` CI (blocking since [PR #817](https://github.com/jackal998/photo-manager/pull/817)) + local Playwright run. The Daylight theme foundation and the results-tree visuals it carries ([#878](https://github.com/jackal998/photo-manager/issues/878) slice (a)). Everything asserted here is **computed style**, which is precisely what vitest cannot reach: the tokens are Tailwind v4 `@theme` colours that only become real utilities once the CSS pipeline runs, and jsdom never runs it — a jsdom assertion on `bg-app` stays green after the token behind it is deleted. Scans `qa/sandbox/near-duplicates/` (5 JPEGs, one group, plain scan) at 1280×800 and asserts: **(1)** the app root computes to `rgb(245, 239, 230)` = `#f5efe6`; **(2)** every mounted similarity badge reports a real `border-style` from {solid, dashed, dotted} and the `(border-style, font-weight)` pairs are **distinct across the states on screen** — Tailwind v4 scans source TEXT for class names, so an interpolated `` border-${style} `` compiles to nothing and the shape cue a colour-blind user depends on vanishes with no test going red — plus the Ref badge still carries its ★ glyph; **(3)** the group header computes to the warm band `#f3e9d8` with a 4px `#bd6b39` accent left strip; **(4)** staging a real delete through the row decision control turns the row background into `#fdf3f0` and the filename's `text-decoration-line` into `line-through`, with the undecided state asserted FIRST so the tint cannot pass by being on every row. Slice (b) adds three more: **(5)** exactly one decision chip is filled — the active `Delete` segment computes to `rgb(196, 80, 63)` on white ink, an untouched row's active `None` segment to the keep chip `rgb(231, 243, 236)` / `rgb(47, 138, 90)`, and every inactive segment to `rgba(0, 0, 0, 0)`; **(6)** the padlock reads faint `rgb(168, 159, 143)` → solid `rgb(189, 107, 57)` across a real click, with `aria-pressed` flipping false → true (so the retone cannot silently cost the lock its action); **(7)** the score mini bar has a fill width `> 0` and `≤` the cell — a `NaN` width is valid-looking markup CSS silently ignores, and a renamed utility compiles to nothing, neither of which jsdom can see. Layout slice R turned (7) around: the fill is now SOLID, so the assertion is `background-image: none` plus the computed colour `rgb(184, 148, 106)`, because the token behind the gradient's second stop is deleted and a surviving gradient utility would paint nothing. Slice R also adds **(11)** the row's geometry at BOTH densities — row height (and the 6px taller last row of a group, a second number in `estimateSize`), the thumbnail box / radius / border, the decision control's inset track and its selected segment, the padlock hit area, the score track, the folder sub-line's left-truncation `direction` and the mono family on a machine-value cell — then flips the `density` localStorage key to `compact`, reloads, and re-reads the whole set. A density-keyed class table is exactly the shape that renders one density correctly and the other not at all. Slice G adds **(12)** the group header's own geometry: `height 44px` at BOTH densities (the band is the one row that does not compress, so it is read again after the density flip), a 14px/700 title, a derived byte total the manifest never sends, and the Q5 button at 28px on the panel surface — plus the one assertion that is not a class at all, the **gap between the button and the caret's hit area**, which is the arithmetic of two live boxes and which Q5 made a requirement because «a misclick from collapse into a bulk decision is the expensive misclick on this screen». Slice E adds **(13)** two things a vitest cannot decide. The similarity legend is read as a POSITION: the pane's scroller is driven to its end and the legend's bottom edge is then compared with the PANE's (gap ≤ 1px), because at scroll-top a legend riding inside the content column looks identical to a pinned one — plus its five states, their 74px min-widths and their distinct grayscale cues. And the execute dialog's all-delete banner is read as COMPUTED colour after every row in the group is staged delete through the row control (the locked row from (6) is unlocked first, which exercises that path a second time): `rgb(253, 243, 217)` fill, `rgb(107, 78, 18)` ink, a `rgb(227, 194, 100)` frame whose LEFT rule is 3px against 1px elsewhere, radius 8px, 13px/500, a leading ▲ and no second ⚠ — the Q9 role in full, where `bg-caution-bg` in jsdom is a string that spells correctly with the token deleted. The five-way similarity mapping itself is pure logic and is pinned by `frontend/src/lib/similarityBadge.test.ts` (five states → five distinct `(glyph, border, weight)` triples); the score-bar clamp by `frontend/src/lib/scoreBar.test.ts` (null / NaN / out-of-domain), and the chip / padlock class mapping by `DecisionControl.test.tsx` + `LockToggle.test.tsx`. |
| `qa/web/scenarios/s75_keep_best_button.py` | `web-scenario-batch` CI + local Playwright run. The "Keep best · delete rest" button open questions **Q5** promoted from the #744 right-click item onto the group header ([#878](https://github.com/jackal998/photo-manager/issues/878) layout slice G). `s72_apply_best_copy.py` already covers the ACTION's manifest semantics through the context menu; this covers the three things Q5 attached to the BUTTON, none of which a unit test can settle. Scans `qa/sandbox/near-duplicates/` at 1280×800, picks a duplicate non-keeper **from the manifest at runtime** (which variant classifies as the ref tier is platform-dependent — s57 D3), locks it through its padlock, then clicks the header button. **(1) It never overrides a lock, with no dialog in the way:** the button sends `skip_locked`, so the locked row keeps BOTH its decision and its lock and the `LockConfirmDialog` never opens — a different flow through the same store action than the right-click item's 409, and only a live run exercises the button's. **(2) The toast says what happened, in the user's language:** Q5 made the toast the replacement for a confirm step, so its sentence is the entire account of a bulk write — the group, the count, «1 locked file unchanged» and Undo, asserted in BOTH locales, because a toast that silently falls back to English is the copy-audit R8 defect one surface over. **(3) Undo actually reverses it:** a jsdom test can prove the button dispatches; only a live run proves the decisions and the locks come BACK — including the lock apply-best-copy put on the keeper, which is the very lock that would refuse the undo's own decision write unless it is forced. Three more live-only checks ride along, each replacing something a unit test would pass vacuously: **(4)** `aria-expanded` is read either side of the button click and must be UNCHANGED, with a row-title click proving the group still toggles at all — Q5's requirement is that the button is not a misclick into a bulk decision, and the px gap that was asserted here first is satisfied by any viewport once the button is flex-pushed right; **(5)** the toast is hit-tested at a point inside its full-width positioning wrapper but outside its box (must NOT resolve to the toast) and at the box's own centre (must), because losing `pointer-events-none` on that wrapper turns it into an invisible click shield over the tree and nothing throws; **(6)** the Shot Date cell is read under both UI locales in one run. The toast's timer, its single-slot replacement and its pause-on-hover are layer 1 (`frontend/src/components/Toast.test.tsx`); the derived byte total and folder suffix are pure functions (`frontend/src/lib/groupSummary.test.ts`); the same-group repeat guard and the locked-count rule are store tests (`useAppStore.test.ts`). |
| `qa/web/scenarios/s76_toolbar_status_density.py` | `web-scenario-batch` CI + local Playwright run. The 52px toolbar and the 30px status strip ([#878](https://github.com/jackal998/photo-manager/issues/878) layout slice TB; closes [#906](https://github.com/jackal998/photo-manager/issues/906)). Five things live here rather than in vitest, each because jsdom cannot decide it. **(1) Computed geometry.** Toolbar `height 52px` with **exactly one** `rgb(168, 90, 44)`-filled control in it — the REPLY's «one filled warm button, so "where do I start" has exactly one answer» is a property of the STRIP, read as a COUNT, so a second accent button fails it; plus the strip must not overflow its own width at 1280px, because its right end is where the only destructive control lives. Status strip `height 30px` at `font-size 11.5px` on `rgb(243, 237, 227)`, and the density control's selected segment on the accent. **(2) The filter against the VIRTUALISED tree.** Rows are windowed, so "hidden" is not a style — the row is not in the DOM at all, and the group header has to go with it. A jsdom filter test proves a predicate ran and nothing more. Asserted in both directions: `q95` leaves exactly the one matching row with its header, a no-match query leaves **0 rows AND 0 headers**, the manifest is byte-identical throughout (the filter is view-only), and clearing restores all five. **(3) The bulk verbs end-to-end.** Ctrl-click two rows through the shared `click_row` / `ctrl_click_row` helpers (a raw centre click lands on the decision control since slice R centred the cells), press the Delete verb, and the MANIFEST has to carry both decisions — then the undo toast, then Undo putting both back. **(4) The density switch and the virtualiser.** Compact re-renders rows at 52px, but `@tanstack/react-virtual` caches a size per index; if nothing re-measures, the total scroll height stays 72px-based and a scroll to the last row lands short. `overscan: 10` hides that near the top, so BOTH halves are read — the row's computed height AND the virtualiser's total size — and then a scroll to the last row. **(5) The filter's reach into the two bulk writes** (round-2 review). Select all five rows, filter to one → the label must read "Set 1 selected:", because the selection survives the filter and a verb pressed under a count of 5 must not write to rows the user cannot see; and the group header's **Keep best** must be DISABLED with its reason in the `title` while filtered, and enabled again once cleared (the false-positive half — a gate that refuses everything looks exactly like one that works). **(6) The narrow-width shed.** At a 1000px viewport the manifest path input and its Open button are absent, and the Delete CTA is REACHABLE — scrolled into view it sits fully inside the viewport and its own centre hit-tests to it — and both controls come BACK at 1280 (a shed that never restores is a removal). Reachable, not zero-overflow: below the design width the strip still scrolls on a wide font stack (measured 113px over at 1000px under a forced fallback), because the only further shed candidates are pinned by owner decision. A CTA you must scroll to and a CTA you cannot reach are different failures. **(7) zh-TW.** The CTA, the delete counter, the filter placeholder and the density segment, with an English-leak check, and `ui.locale` restored to `en` in a `finally` because it is a SERVER-side setting (the s75 lesson: leaking it times out every English-reading scenario after this one). The filter predicate, the two totals, the source-name derivation, the bulk verbs' lock split and the label interpolation are all layer 1 (`rowFilter.test.ts`, `deleteTotals.test.ts`, `scanSummary.test.ts`, `Toolbar.test.tsx`, `StatusBar.test.tsx`, `ResultTree.filterDensity.test.tsx`, `useAppStore.test.ts`). |
| `qa/web/scenarios/s77_preview_pane.py` | `web-scenario-batch` CI + local Playwright run. The preview pane ([#878](https://github.com/jackal998/photo-manager/issues/878) layout slice PV; closes [#905](https://github.com/jackal998/photo-manager/issues/905)). Five things live here rather than in vitest. **(1) Computed geometry.** The pane's own width is read off the LIVE box and must be 320px — the width is a property of the flex row (tree + 4px handle + pane), so a constant of 320 and a pane of 288 are both possible at once. The image frame's ASPECT is read as a real ratio from its bounding box (4/3 ± 0.02) rather than from the `aspect-ratio` string, which can be present while a stretching parent overrides it — and an overridden aspect is [#535](https://github.com/jackal998/photo-manager/issues/535)'s portrait oscillation coming back, which the always-on scrollbar only damps. Plus radius 12px, `overflow: hidden`, `object-fit: contain`, the `#24211e` surround as `rgb(36, 33, 30)`, and the title strip at 42px / 11px / uppercase. **(2) The similarity badge sits OVER the surround** — present, and its box inside the frame's, not above it in flow. **(3) The metadata table is exactly five rows** (Name · Folder · Size · Resolution · Shot date) with every value right-aligned in the mono stack; seven rows is what the pane shipped with, and the COUNT is the assertion because an extra row is how a 320px pane silently becomes a scroller. **(4) #905 end-to-end.** Clicking Delete in the PANE turns the pane's own button selected, the ROW's chip to delete, and the MANIFEST to `'delete'` — two surfaces, one store action; a vitest can prove the pane dispatches, only a live run proves the row re-renders from the same state. **(5) The lock gate on a REAL lock.** Locking through the row's padlock is a server round trip, and the pane's three buttons must come back disabled from the state that write produced — with the false-positive half (unlock, assert enabled again), because buttons disabled for everyone look exactly like a gate that works. Riding along: the keep-worthiness bar's 7px track, a fill width `> 0` and `≤` the track (a NaN width is valid-looking markup CSS ignores) with `background-image: none` (the gradient is retired and its second stop's token is gone, so a surviving gradient utility paints nothing); and zh_TW for the title, all five metadata labels, the three decision words (which must be the row control's, from the one `DECISION_VOCAB` table) and the Shot date read under BOTH UI locales in one run, with `ui.locale` restored to `en` in a `finally` (it is a SERVER-side setting — the s75 lesson). The width migration, the five-row composition, the decision dispatch and the lock-disabled state are layer 1 (`frontend/src/lib/panelWidths.test.ts`, `frontend/src/components/PreviewPane.test.tsx`). |
| `qa/web/scenarios/s78_empty_state.py` | `web-scenario-batch` CI + local Playwright run. The empty state ([#878](https://github.com/jackal998/photo-manager/issues/878) layout slice E). No scan and no fixture — the empty state IS the app with nothing loaded, which makes it the cheapest scenario in the batch. At 1280×800: **(1)** the primary CTA is actually primary — computed height 48px, radius 12px, a 15px label, `rgb(168, 90, 44)` on white, where the screen shipped with two identical 32px neutral buttons and in jsdom `h-12` / `bg-warm` are strings that spell correctly with the token deleted; **(2)** EXACTLY ONE accent-filled button on the screen, asserted as a COUNT the way s76 asserts the toolbar's, because a second primary is a property of the screen rather than a class regression on either button; **(3)** the 25px/700 heading (audit E3 — a heading rendered at the body's 14px is the defect filed) and the column's 520px cap (E1), both read off live boxes; **(4)** two owner decisions visible only as ABSENCE — the dropped Q10 safety line and the un-built E7 recent-sources list, checked in BOTH locales, since absence is indistinguishable from "not shipped yet" and the prototype draws both; **(5)** the composition changed but the handlers did not (clicking the primary opens the real Scan dialog); **(6)** the same screen in zh_TW, where the heading and body are NEW keys — exactly what gets added to en.yml alone and renders English inside a Chinese session (copy audit R8) — with the CTA keeping its 48px box under a different label, and `ui.locale` restored to `en` in a `finally` (a SERVER-side setting; leaking it times out every English-reading scenario after this one — the s75 lesson). The composition itself is layer 1 (`frontend/src/App.test.tsx`). |
