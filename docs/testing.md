...
| `app/web/routes/media.py` | `tests/test_web_media_route.py` (94%) | `GET /api/media` — raw-byte streaming endpoint with HTTP Range/206 support for video (and generic media) files. Reuses `_path_guard.validate_under_roots` for 400/403 path guard; 404 when file absent; 416 on unsatisfiable or malformed range. No Range → 200 full file + `Accept-Ranges: bytes`; `Range: bytes=START-END` → 206 + `Content-Range` + exact body (END inclusive per RFC 7233); open-ended `bytes=START-` → 206 to EOF; suffix-range `bytes=-N` → 206 last N bytes; degenerate `bytes=-` / `bytes=-0` → 416. Content-Type resolved from extension with an explicit map for types Python's `mimetypes` gets wrong (`.mov` → `video/quicktime`, `.webm` → `video/webm`). Streams in 1 MiB chunks so large videos never fully load into RAM. Layer-1 covers all status-code branches, RFC math (inclusive END, Content-Length = END-START+1), suffix-range math and edge-cases, 6 extension→Content-Type mappings, and path-guard rejection. Layer-3: `qa/web/scenarios/s69_video_playback.py` (browser decode + playback; fixture `qa/sandbox/video-playback/clip.mp4`, VP9-in-MP4, 2 s). |
