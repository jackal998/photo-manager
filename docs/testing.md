...
| `app/views/theme.py` | 100% | render harness `scripts/render_ux_baseline.py` (visual) | Daylight·light tokens + QSS (feat/review-ux-daylight Phase 1). `tests/test_theme.py` guards token/template drift — a `@token@` in the template missing from `DAYLIGHT` would leave a broken QSS rule; `apply_theme` install verified on the session app. Pure string substitution + one `setStyleSheet` call; no behaviour. |
| `app/views/density.py` | 100% | s-N/A (visual relayout) | Process-global Comfortable/Compact row-height state (Phase 2e). `tests/test_density.py` covers default / set / unknown-fallback / available, and that a delegate's `sizeHint` height actually changes with density (the load-bearing link to the View → Density menu). |
| `app/views/delegates/similarity_badge.py` | 83% | render harness (visual) | 5-state similarity pill painter (Phase 2b). `tests/test_similarity_badge.py` pins the classification gate (group rows / role-less cells fall back; Ref gets the ★) and renders the badge to a QImage asserting the fill lands (catches a silently non-painting delegate). Uncovered: hover/selection-bg branches + the dashed-border branch. |
| `app/views/delegates/score_bar.py` | 84% | render harness (visual) | Keep-worthiness mini-bar + number (Phase 2c). `tests/test_score_lock_delegates.py` covers the score helper (fraction / None / group-row), the sizeHint widen, and renders warm-fill-present for a scored row / absent for an unscored "—" row. |
| `app/views/delegates/lock_icon.py` | 85% | render harness (visual) | Painted padlock replacing the 🔒 emoji (Phase 2c). `tests/test_score_lock_delegates.py` covers the lock helper + renders accent present only when locked. |
| `app/views/delegates/decision_control.py` | 82% | render harness (visual); s-N/A click | Inline Keep/Delete/Remove control (Phase 2d). `tests/test_decision_control.py` covers the 3-segment model, the `''`→Keep active mapping, the segment rects, a simulated `editorEvent` click → `decisionPicked` with the right value (delete + ignore), the group-row no-op, and that the active Delete segment is filled red. **Layer-3 gap:** a UIA click on a row segment isn't yet scripted — the click hit-test is pinned at L1 via the synthesized `QMouseEvent` instead. |
| `app/views/components/toolbar_builder.py` | 97% | render harness (visual) | Main toolbar (Phase 3). `tests/test_toolbar_builder.py` verifies it reuses the supplied menu actions, adds exactly three bulk actions wired in order, tags Scan/Execute with the styling object names, and tolerates a sparse action dict. |
| `app/views/components/similarity_legend.py` | 100% | render harness (visual) | Static preview-footer legend (Phase 4). `tests/test_similarity_legend.py` checks all five swatches are present, the passenger pill is dashed (others solid), and the swatch reuses the badge `_BADGE_STYLES` colours. |

> **feat/review-ux-daylight note:** the results-tree visual redesign is
> delegate + QSS based (Phases 1–4); the per-state painting is verified by
> unit render-to-QImage assertions above plus the `render_ux_baseline.py`
> before/after harness. `tree_model_builder.py` / `tree_controller.py` gained
> the `DECISION_ROLE` / `SIMILARITY_KIND_ROLE` contract the delegates read
> (pinned by `TestSemanticRoles`). The one acknowledged layer-3 gap is a
> scripted UIA click on an inline decision segment — tracked as follow-up.
