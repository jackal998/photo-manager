...
| `scanner/autotune.py` | 100% | — | #551 Phase-0 manual run | Pure occupancy-sampler logic (`OccupancyProbe`: EWMA update, regime classification, starved-count). Covered at layer 1 by `tests/test_autotune.py` (15 cases: compute-bound / io-bound / mixed streams, starved-count accumulation, fail-safe on maxsize=0, EWMA arithmetic). The env-gated pipeline wiring in `scan_worker.py` (`PHOTO_MANAGER_AUTOTUNE_PROBE`) is covered manually by the #551 Phase-0 validation run, not in CI — marked `# pragma: no cover` per the same convention as `disk_incurs_seek_penalty`'s Win32 IOCTL boundary. |
