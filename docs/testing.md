...
| `scripts/check_features_scenario_citations.py` | **omit** (scripts/*) | Guards `docs/features.md`: every `qa/web/scenarios/` citation must name a file that exists, and zero citations may point at the removed desktop tree. Added in [#646](https://github.com/jackal998/photo-manager/issues/646) — on PR A's tip it reports 133 dead citations. Run it after renaming or deleting a driver. |
| `scripts/check_testing_doc_modules.py` | **omit** (scripts/*) | Guards THIS document: every module row in the per-module map above must resolve against `git ls-files`. Added in [#646](https://github.com/jackal998/photo-manager/issues/646), where the cutover left 36 rows describing coverage for files that no longer existed — a stale row reads as coverage. Run it after moving or deleting a module. |
