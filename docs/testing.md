...
| `app/views/tree_model_builder.py` | 76% | s01, s06, s07, s10, s46 (#165 prototype — `grey_undecided=True` is reached when the main window's `_sync_execute_mode_ui` refreshes the model after the Ctrl+E toggle, so file rows with empty `user_decision` render in mid-grey) | uncovered 24% is `setData()` `except: pass` defensive wrappers — only triggered if Qt's setData raises, which doesn't happen in practice. Lock column (#182, supersedes #164's glyph-prefix-on-Action approach) pinned by `TestActionDisplayUnaffectedByLock` (Action returns just the decision label regardless of lock state), `TestLockDisplay` (🔒 / empty rendering), and `TestLockColumnInBuiltModel` (end-to-end via `build_model` — locked row has 🔒 in COL_LOCK and bare decision in COL_ACTION; SORT_ROLE on COL_LOCK is 0/1 so the column is sortable). #165 `grey_undecided` flag pinned at layer 1 by `tests/test_tree_model_builder.py::TestGreyUndecided` (default False leaves foreground untouched; undecided rows in grey-mode get a `QBrush` on every cell; decided rows are not touched). |
...
| `app/views/workers/scan_worker.py` | 83% | every scan scenario; s49 covers the new auto-select branch (#212 — top-scored row promoted to `action="KEEP"` before manifest write) | minor — cancellation timing branch hard to test deterministically. The auto-select branch itself is gated by a constructor flag; the unit tests in `tests/test_auto_select.py` pin the underlying decision logic and s49 pins the in-process wiring end-to-end. |
...
| `app/views/components/menu_controller.py` | 89% | s01, s18, s21, s22, s28, s46 (#165 prototype — View → Execute Mode toggle, checkable + Ctrl+E shortcut, gated on a loaded manifest via `MANIFEST_ACTIONS`); s14 (#244 — `assert_action_menu_gated_pre_manifest` step verifies both `Set Action by Field/Regex…` and `Execute Action…` start greyed before any manifest loads) | uncovered 11% is fallback branches in the language picker (no available locales) and a defensive guard for missing manifest-actions; the View → Language exclusivity + Yes/No confirm + dirty-flag exit prompt all unit-tested in `test_menu_controller_manifest_actions.py`. The #165 prototype's `execute_mode` action is pinned at layer 1 by `tests/test_execute_mode_helpers.py::TestExecuteModeMenuAction` (checkable, Ctrl+E, manifest-gated). The #244 pre-manifest gating is also pinned at layer 1 by `tests/test_ui_probes.py::test_probe_manifest_dependent_menu_actions_are_gated` (AST inspection of `MANIFEST_ACTIONS` membership). |
...
| `app/views/main_window.py` | not measured (no test imports it; QMainWindow assembly cascades into the whole view stack) | every scenario constructs MainWindow as a real subprocess; #141 geometry round-trip is layer-3 via s39 (window_state.ini round-trip across launches); #214 column-layout round-trip is layer-3 via s47 (same window_state.ini, separate key); close-event dirty-prompt logic is layer-3 via s28; #137 empty-state action buttons via s41 (the construction-time `build_empty_state_widget` call); s46 covers the #165 prototype's Execute Mode toggle round-trip (title suffix + action bar + menu indicator) | window-state persistence (#141, #214, #215) is intentionally NOT mock-tested — the save/restore paths are uniform plumbing around Qt's own `saveGeometry` / `restoreGeometry` / `QHeaderView.saveState` / `restoreState`, and mocking QSettings to "cover" them would be metric gaming per CLAUDE.md. s39, s47, and s48 round-trip real QSettings through a real `closeEvent` (or `QDialog.done()`) and a real re-launch / re-open, which is what actually catches a regression. Same shape for #137: the click-wiring contract is layer-1 in `app/views/components/empty_state.py`'s tests; s41 only asserts MainWindow actually uses the builder and reaches the right dialog from each button. #165 prototype: `complete_delete_group_numbers` (banner content driver) lives in the sibling `app/views/execute_mode_helpers.py` so the layer-1 test can import it without cascade-loading the view stack (PreviewPane / ImageTaskRunner / DialogHandler / GroupMediaController / VideoPlayer have no layer-1 coverage and aren't measured under master); pinned at layer 1 by `tests/test_execute_mode_helpers.py::TestCompleteDeleteGroupNumbers`; s46 covers the visible UI signals (title suffix, action-bar visibility, menu checkmark) round-trip via Ctrl+E. |
