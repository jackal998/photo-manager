...
| **3 — QA / E2E** (real GUI) | Label drift, state-transition bugs, layout regressions, end-user flow failures | **Web client — the gate:** `web-eval-gates.yml` on **every** PR (Playwright `qa/web/scenarios/`, plus web-dom-probe / qa-parity-counter / bench-sanity); locally `python -m qa.web._batch`. **Qt client — legacy:** `qa-batch.yml` only on PRs touching a **Qt surface** (the `paths:` allowlist in that workflow — `app/views/**`, `app/viewmodels/**`, `qa/scenarios/**`, `qa/probes/**`, `qa/sandbox/**`, `translations/**`, `main.py`, `launcher.py`, the two sandbox generators, the PySide6/pywinauto pins, and the workflow itself); locally `python -m qa.scenarios._batch` | Web: **BLOCKING** since [PR #817](https://github.com/jackal998/photo-manager/pull/817) ([#773](https://github.com/jackal998/photo-manager/issues/773)); 71/71 scenarios ported per `check_qa_parity.py --phase 4` — this is what guarantees the shared backend (`core/`, `scanner/`, `infrastructure/`) is exercised end-to-end on every PR. Qt: **NON-REQUIRED, informational**, scoped to Qt-surface PRs by [#863](https://github.com/jackal998/photo-manager/issues/863) and deleted with the Phase-4 cutover ([#646](https://github.com/jackal998/photo-manager/issues/646)); [#74](https://github.com/jackal998/photo-manager/issues/74)'s "promote to required after ≥10 flake-free runs" plan is superseded. Accepted gap: a shared-backend change that breaks only the Qt client waits for the next Qt-surface PR or a manual `workflow_dispatch` |
...
| `infrastructure/transcode_service.py` | `tests/test_transcode_service.py` (≥70%) | CI-safe: cache-key determinism, mtime-sensitivity, cache-hit short-circuit (no ffmpeg), per-key lock, atomic rename naming, and (#854) binary resolution — bundled-beside-the-frozen-exe vs `sys._MEIPASS` vs PATH vs nothing, plus H.264 encoder selection from a faked `ffmpeg -encoders` listing (the bundled LGPL build has no libx264). Real ffmpeg encode is gated by `tests/integration/test_transcode_integration.py` (runs in `bench-sanity` CI job via `apt-get install ffmpeg`). Residual risk: concurrent Semaphore(2) deadlock on exception is unit-tested; real NAS latency / disk-full / corrupted ffmpeg output are dev-rig verifiable only. |
...
| `scripts/bench_web_port.py` | **omit** (scripts/*) | web-port cutover gate: Qt-vs-web scan-throughput A/B (T6). Asserts each arm's `files_per_s > 0` (liveness); `--require-ratio` adds the web/qt ≥ 0.95 floor on a real corpus. Run in CI by the **`bench-sanity` job** (blocking since PR #817) in `web-eval-gates.yml` (`--backend web` — the Qt arm is a local dev-rig A/B checkpoint, Qt platform libs absent in CI). |
| `scripts/preview_phase3_analysis.py` | **omit** (scripts/*) | #622 Phase 3 instrument, pure half. Fully covered at layer 1 by `tests/test_preview_phase3_probe.py`: file discovery + click order, the steady-state window, the box-2 ratio maths (including every exclusion that keeps a non-comparison out of the pairing, and the `dropped_timeouts` accounting), the box-1 verdict in BOTH directions, the box-1 *refusals* (all-placeholder window, all-timeout window, too-few cold decodes) together with the legitimate cache-heavy run that must still get a verdict, `overall_verdict`'s NOT_MEASURED-outranks-PASS ordering, the citation-field schema check, and the LF-only artifact write. These are the functions whose silent breakage would put a wrong number into a citation, which is why they are a separate module from the harness below. |
| `scripts/preview_phase3_probe.py` | **omit** (scripts/*) | #622 Phase 3 instrument, Qt harness half (`run_session` / `run_modal_check` — real `ImageService`, `ImageTaskRunner`, `PreviewPane`, `FullResViewerDialog`; only `MainWindow` is stood in for). **Layer 3 / manual**: it needs a QApplication, a real image tree and a real decoder, so it is exercised by running the probe per `docs/audits/preview-phase3-runbook.md`, not from CI. **Residual risk:** the `embedded_jpeg` and `forced_full_decode` routes both require real DNG files, which neither CI nor `qa/sandbox` has — a local smoke run reaches `non_raw_source` and `cache_hit` only, and box 2 correctly reports `not_measured` there, so those two route labels are first exercised during the owner's NAS session. The 1-ahead prefetch is out of the harness's scope by design (it fires on group selection in `MainWindow._prefetch_next_group`, not on a single-file click). |
...
| `s48` (web) `probe_status: s48 <dialog> open_animation=…` (soft, live) | An opened dialog reports a real `animationName` + duration — i.e. the shared wrapper's `animate-in` / `zoom-in-95` classes still compile to CSS | PASS (informational — prints, never fails) | Forward-defensive against [#852](https://github.com/jackal998/photo-manager/issues/852) recurring: the `tw-animate-css` import being dropped again. No unit layer can see this — jsdom has no animation CSSOM — so a live browser reading is the only observation that exists. Prints no `PASS` token, so the promotion gate does not apply: there is no bug to promote it to, only a fact to keep visible. |
