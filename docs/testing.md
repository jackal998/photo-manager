...
| `scanner/dedup.py` | 93% | — | s01, s07, s10, s11 | low — pure logic, well-covered. Internal `rows` and `path_to_hr` dicts are str-keyed (not `Path`-keyed) so genuinely-distinct files differing only by filename case (case-sensitive NTFS dirs; rare) survive — see #170. `TestCaseSensitiveCollision` pins this. Live Photo HEIC+MOV/MP4 pairs always share a group_id via pair edges fed into union-find — `TestLivePhotoPair` covers the unique-pair-forms-group case (#88 headline regression), the asymmetric case where each side is in its own SHA-group, and the per-row independence of action / user_decision when the HEIC is itself a duplicate. s11_video_live drives the same invariant end-to-end through the GUI. |
...
|---|---|---|---|---|
| `infrastructure/manifest_repository.py` | 99% | — | every scenario | very low. `is_locked` column persistence (#164) is round-tripped in `TestIsLockedPersistence` (4 cases): default-False on load, additive migration on pre-#164 DBs, ``batch_update_lock_state`` write+read, and unlock-after-lock. |
...
| `app/views/tree_model_builder.py` | 76% | s01, s06, s07, s10 | uncovered 24% is `setData()` `except: pass` defensive wrappers — only triggered if Qt's setData raises, which doesn't happen in practice. `_action_display`'s lock-glyph branch (#164) pinned by `TestActionDisplayLockGlyph` including an end-to-end `build_model` test that finds the 🔒 prefix in COL_ACTION for a locked record. |
...
| `app/views/handlers/file_operations.py` | 85% | s01 + every scenario that loads a manifest; s12 for Save Manifest Decisions; s14 / s29 / s30 / s31 exercise the bulk-regex apply path through `set_decision_by_regex` (delete, remove-from-list deferred, right-click route, Simple-mode synthesis); `build_match_fn` covered by `TestBuildMatchFn` + every regex-dialog driver via the live preview | uncovered 15% is QFileDialog interaction (file picker for open manifest) and a few rarely-hit error branches in the manifest open/save callbacks. Lock semantics (#164) pinned at layer 1: `TestSetLockedState` (write/read-back, idempotent re-lock, unlock), `TestSingleRowOverridesLock` (single-row right-click bypasses skip-locked), `TestSetDecisionByRegexSkipsLocked` (destructive regex skips locked rows; lock/unlock sentinels apply to all matched). |
...
| `app/views/dialogs/execute_action_dialog.py` | 83% | s13 (real send2trash through the GUI), s30 (Phase A right-click parity — opens the regex dialog from the Execute tree's context menu), s33 (#166 — banner renders the flagged group number) | uncovered 17% is the actual destructive `_on_execute` flow + a few error branches in the path-not-found dialog; s13 covers the destructive happy path. Spot-add a layer-2 test only if a destructive-flow bug surfaces that's hard to reproduce via the GUI. Lock at execute stage (#164) pinned by `TestExecuteDialogLock`: single-row `_set_lock`, `_set_decision` LOCK / UNLOCK sentinel routing, regex destructive skip-locked, regex LOCK / UNLOCK applies to all matched, all-locked surfaces a dedicated dialog. Banner jump-to (#166) pinned at layer 1 by `TestBannerJumpTo`: anchor rendering, `_on_jump_to_group` selects the matching group row, invalid/unknown hrefs are no-ops; the QLabel HTML-anchor click itself isn't UIA-clickable, so s33 only verifies the banner renders the right group number — the click → scrollTo dispatch stays unit-tested. |
