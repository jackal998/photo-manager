...
| `app/views/dialogs/execute_action_dialog.py` | 83% | s13 (real send2trash through the GUI), s30 (Phase A right-click parity — opens the regex dialog from the Execute tree's context menu) | uncovered 17% is the actual destructive `_on_execute` flow + a few error branches in the path-not-found dialog; s13 covers the destructive happy path. Spot-add a layer-2 test only if a destructive-flow bug surfaces that's hard to reproduce via the GUI. Lock at execute stage (#164) pinned by `TestExecuteDialogLock`: single-row `_set_lock`, `_set_decision` LOCK / UNLOCK sentinel routing, regex destructive skip-locked, regex LOCK / UNLOCK applies to all matched, all-locked surfaces a dedicated dialog. |
