...
| `app/views/dialogs/scan_dialog.py` | 90% | every scenario opens it; s17 (full source-list operations); s38 (#144 — inline error when typed path doesn't exist, error clears on next add) | uncovered 10% is QFileDialog browse interaction + a few worker-signal branches. The path-field validation surface (`_on_add_typed` + `_clear_path_error`) is pinned at layer 1 by `TestPathFieldEntry` (empty no-op, non-existent path → visible error label naming the path, keystroke clears stale error, valid path emits + clears prior error) with s38 mirroring end-to-end via UIA against the live dialog — without s38 a regression that broke the QLabel's accessible-name surface would pass layer 1 but the user still wouldn't see the error in the real dialog. |
