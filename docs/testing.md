...
| `app/views/components/menu_controller.py` | 89% | s01, s18, s21, s22, s28, s46 (#165 prototype — View → Execute Mode toggle, checkable + Ctrl+E shortcut, gated on a loaded manifest via `MANIFEST_ACTIONS`) | uncovered 11% is fallback branches in the language picker (no available locales) and a defensive guard for missing manifest-actions; the View → Language exclusivity + Yes/No confirm + dirty-flag exit prompt all unit-tested in `test_menu_controller_manifest_actions.py`. The #165 prototype's `execute_mode` action is pinned at layer 1 by `tests/test_execute_mode_helpers.py::TestExecuteModeMenuAction` (checkable, Ctrl+E, manifest-gated). |
