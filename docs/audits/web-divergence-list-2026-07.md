...
| D19 | Result tree — roving arrow-key focus | ArrowDown/ArrowUp move focus/selection through the tree after manifest load (Qt s26 steps 1/3), selection preserved across model rebuilds | **RESOLVED (#709)** — ArrowDown/ArrowUp rove the **visible row order** (group headers are stops, as in the Qt `QTreeView`), writing the same store updates a click does so the preview pane and the `d`/`k` shortcuts follow. Landing on a group header selects the group and CLEARS the file selection, matching Qt's `set_decision_to_highlighted` filter (`type=="file"` over the current selection) so `d`/`k` are no-ops there instead of writing to the row the cursor left; a right-click and the post-scan keeper scroll both move the cursor too, so the next arrow continues from what the user last acted on. Because the rows are virtualized, absolutely positioned and non-focusable, the **scroll container** is the focus target (`tabIndex=0`, `role="tree"`) and `aria-activedescendant` names the active row's `role="treeitem"` wrapper — DOM focus never rides on a row that can be unmounted. The cursor is held as a row IDENTITY, so it survives a `manifest.groups` rebuild (the Qt "preserved across model rebuilds" half). The active row is kept on screen with `scrollToIndex(…, {align:"auto"})` over a new `scrollPaddingStart` = #699's measured `scrollMargin`, so a row scrolled in from above lands at the sticky header's bottom edge instead of underneath it. Still out of scope (QTreeView built-ins, not #709): Shift+arrow ranges, Home/End/PageUp/PageDown, Left/Right collapse | resolved | Shipping anchors: `frontend/src/components/ResultTree.tsx` (`ActiveRow` / `rowDomId` / `activeIndex` / `activateIndex` / `handleKeyDown`, `scrollPaddingStart`, `tabIndex`+`role="tree"`+`aria-activedescendant` on the tree root, `id`+`role="treeitem"` per row wrapper); `frontend/src/components/ResultTree.keyboard.test.tsx`; `qa/web/scenarios/s26_keyboard_navigation.py` (branch D) + its `qa/web/scenario_map.yml` note; `docs/features.md` "Web parity — roving arrow-key cursor (#709)". Live evidence (headless Chromium, window shrunk to 1280x420 so the 5-row fixture scrolls): the pre-fix build fails branch D at `tabindex=None role=None`; on head, ArrowDown x2 lands on `neardup_02_q80.jpg` with the preview src following, and ArrowUp back onto the off-screen first row reports `clearance=0.00px` against the sticky header (a build without `scrollPaddingStart` reports the row 28px UNDER it — mutation-probed in the unit suite) | N/A — resolved, shipped #709 (owner SIGNED 2026-07-17 as tracked-polish); no longer a divergence |
