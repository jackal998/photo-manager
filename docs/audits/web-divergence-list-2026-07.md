...
| D23 | Preview/cache — per-device request coordination + prefetch (#622 Phase 2) | N/A (new architecture, not a Qt-side behavior being ported) | Viewport-bounded byte-budget cache (#622 Phase 1) shipped; the `PreviewRequestCoordinator` per-device serialization/cancellation-token layer and 1-ahead prefetch (Phase 2) do not exist yet | open-polish | Issue [#622](https://github.com/jackal998/photo-manager/issues/622) "How" §Phase 2; verified live — `app/views/preview_coordinator.py` and `infrastructure/device_key.py` do not exist on disk | OPEN |
| D24 | Preview/cache — real-NAS performance verification (#622 Phase 3) | N/A | Phase 1's NAS-bandwidth claims (embedded-JPEG fast path, byte-budget LRU) are architecturally shipped but not yet verified against a real NAS with a 4-tuple citation (probe + SHA + args + JSON) per the project's perf-claim discipline | open-polish | Issue #622 "How" §Phase 3 ("real-NAS verification with 4-tuple citation … per `feedback_perf_claim_must_cite_artifact.md`") | OPEN |

## Row count by class and sign-off state

| Class | Count | SIGNED | OPEN | N/A |
|---|---|---|---|---|
| endorsed-omission | 4 (D1–D4) | 3 (D1, D3, D4) | 0 | 1 (D2, resolved/shipped) |
| n-a-by-design | 8 (D5–D11, D15) | 0 | 7 (D5–D11) | 1 (D15) |
| accepted-divergence | 3 (D12–D14) | 1 (D13) | 2 (D12, D14) | 0 |
| open-polish | 9 (D16–D24) | 0 | 9 | 0 |
| **Total** | **24 rows** | **4** | **18** | **2** |

## OPEN rows for owner decision

- D5 — File → Exit menu item (n-a-by-design; likely a quick keep)
- D6 — Execute dialog window-geometry persistence (n-a-by-design; likely a quick keep)
- D7 — Set Action dialog window-geometry persistence (n-a-by-design; likely a quick keep)
- D8 — Main window geometry+splitter persistence, chrome half (n-a-by-design; likely a quick keep)
- D9 — Main window dirty-flag exit prompt (n-a-by-design; likely a quick keep)
- D10 — Row-selection horizontal auto-scroll suppression (n-a-by-design; likely a quick keep)
- D11 — Developer memory probe (n-a-by-design; likely a quick keep)
- D12 — FullResViewer resizable/movable overlay window (accepted-divergence, explicitly awaiting sign-off per features.md — the substantive owner decision in this batch)
- D14 — Singleton-group display architecture (accepted-divergence per audit judgment, not yet owner-signed)
- D16 — Secondary sort by score within groups (#743 sub-item)
- D17 — Group-header double-click expand/collapse (#743 sub-item)
- D18 — Empty-area (tree background) right-click menu (#743 sub-item)
- D19 — Roving arrow-key focus in ResultTree (#709)
- D20 — Remove-after-Unlock&Apply live QA coverage (#713 / #702 item 4)
- D21 — Virtualizer scrollMargin coordinate offset + tall-manifest QA coverage (#699)
- D22 — Per-dimension scoring signal exposure (#680)
- D23 — PreviewRequestCoordinator (#622 Phase 2)
- D24 — Real-NAS perf verification (#622 Phase 3)
