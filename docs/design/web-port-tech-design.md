...
**Status:** DRAFT v0.1 -- pending `/adversarial-review` on the targets in Appendix B before the contracts are locked.
...
| **4** | Cutover | (a) `scripts/check_qa_parity.py --phase 4 --require-all` passes (all 64 scenarios `status: done`). (b) Playwright `web-playwright-batch` CI job green (the Playwright equivalent of `qa-batch.yml`, same shard model, same `fail-fast: false`, running on `ubuntu-latest` headless). (c) `bench_web_port.py --pairs 3` ratio ∈ [0.95, 1.20] on synthetic-2000. (d) `bench_thumbnail_latency.py` p95 ≤ 200. (e) `WEB_PORT_PHASE=4` set in repo vars. (f) `qt qa-batch` removed from required status checks (replaced by `web-playwright-batch`). | All gates blocking; no merging without all 6 |
