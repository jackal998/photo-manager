...
# Device identity lives in infrastructure/device_key.py (#622 Phase 2) so the
# preview coordinator can bucket per device without importing this module.
# Re-exported here — unchanged names, one implementation — so every existing
# caller (core/app_service/scan_runner.py, the tests) keeps working.
from infrastructure.device_key import (  # noqa: F401 — re-export
    _DRIVE_REMOTE,
    _extract_server,
    _resolve_unc_via_win32,
    _server_key_for_letter,
    _unc_cache,
    device_key,
    is_remote_drive,
)
