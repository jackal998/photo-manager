...
# ── grey_undecided flag (#165 Execute Mode prototype) ──────────────────────


class TestGreyUndecided:
    """The flag stamps a foreground brush on every cell of any file row
    whose ``user_decision`` is empty. Decided rows (delete / keep / etc.)
    keep the default render so the user reads which rows still need
    action at a glance.
    """

    def test_default_false_leaves_foreground_unset(self):
        """Without the flag, file-row cells carry no explicit foreground."""
        from PySide6.QtCore import Qt

        from app.views.tree_model_builder import build_model

        undecided = _rec(file_path="/p/und.jpg", user_decision="")
        g = _group([undecided])
        model, _ = build_model([g])
        group_item = model.item(0, 0)
        child_name = group_item.child(0, 0)
        # QStandardItem.data(ForegroundRole) returns None / invalid when
        # no brush has been stamped — Qt then renders with the palette
        # default. That's the contract being asserted here.
        assert child_name.data(Qt.ForegroundRole) in (None,)

    def test_undecided_row_gets_grey_brush(self):
        """When grey_undecided=True, every cell of an undecided file row
        carries the mid-grey foreground brush. Decided rows are not
        touched in the same model."""
        from PySide6.QtCore import Qt
        ...
        undecided = _rec(file_path="/p/und.jpg", user_decision="")
        decided = _rec(file_path="/p/dec.jpg", user_decision="delete")
        g = _group([undecided, decided])
        model, _ = build_model([g], grey_undecided=True)
        group_item = model.item(0, 0)

        # Row order matches the input order — first child is undecided.
        und_row_brush = group_item.child(0, 0).data(Qt.ForegroundRole)
        dec_row_brush = group_item.child(1, 0).data(Qt.ForegroundRole)

        assert isinstance(und_row_brush, QBrush)
        # Decided row gets no explicit brush — falls back to palette.
        assert dec_row_brush in (None,)

    def test_every_column_of_undecided_row_carries_brush(self):
        """Selective columns would render a half-grey row. Assert that
        the brush is stamped on every cell of the undecided child row,
        not just the first."""
        from PySide6.QtCore import Qt
        from PySide6.QtGui import QBrush
        ...
        und = _rec(file_path="/p/und.jpg", user_decision="")
        g = _group([und])
        model, _ = build_model([g], grey_undecided=True)
        group_item = model.item(0, 0)
        for col in range(NUM_COLUMNS):
            brush = group_item.child(0, col).data(Qt.ForegroundRole)
            assert isinstance(brush, QBrush), f"column {col} missing brush"
