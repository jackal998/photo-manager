...
1. **`qa_scenario_guard`** (PreToolUse on `gh pr create`) — fires on
   `frontend/src/` and `app/web/` diffs without
   a `qa/web/scenarios/sNN_*.py` change. Bypass:
...
2. **`docs_guard`** (PreToolUse on `gh pr create`) — has **two
   ...
   - **new-file trigger**: NEW `.py` files under the structured
     dirs (`app/web/`, `frontend/src/`,
     `infrastructure/`, `scanner/`, `core/{models,services/}`,
     `tests/test_*.py`, `qa/web/scenarios/s*.py`). Satisfied by ANY
     edit to `README.md` / `docs/*.md` / `CLAUDE.md` /
     `pyproject.toml`.
   - **behavioural-modify trigger** (#262): MODIFIED files under
     `app/web/routes/` and `frontend/src/components/` with ≥10 added+deleted lines
...
trigger** (MODIFIED `app/web/routes/*.py` and `frontend/src/components/**.tsx` over the
...
`frontend/src/App.tsx` and `frontend/src/hooks/*` (OUTSIDE the
behavioural-modify roots), so they never triggered docs_guard's
strict mode. #293 targeted `app/web/routes/execute.py`
(INSIDE a strict root), which DID trigger, and the brief's
...
Universal — bake into any brief that adds qa/web/scenarios coverage.
