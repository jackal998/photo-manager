...
1. **Upstream callers** — `Grep` the symbol name across `app/`, `scanner/`,
   ...
   under `qa/web/scenarios/`, doc tables in `docs/testing.md`, and string
   references in `frontend/src/` (button labels, menu titles, accessibility
...
2. **Downstream dependencies** — what does the function call into? Note any
   I/O (DB, filesystem, subprocess, HTTP) it relies on, since changes there cascade.
...
5. **Plan test updates** — if behavior is changing, list the existing tests
   ...
   **What a real test looks like** (per [`CLAUDE.md`](../../../CLAUDE.md)
   ...
   a store setter to raise just so the wrapped `except: pass`
...
|-------------------------------------------------|---------------------------------------------------|--------------------------------------------|
| frontend/src/components/execute/ExecuteDialog.tsx:127 | frontend/src/components/execute/ExecuteDialog.test.tsx (L1)        | none                                       |
...
| qa/web/_invariants.py:55 (button title constant) | qa/web/scenarios/sNN_*.py drivers (L3)                                 | update constant + commit message; layer-3 batch will catch drift on next run |
```
